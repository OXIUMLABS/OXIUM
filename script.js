...
function showToast(msg) {
  const el = document.getElementById("toast"); el.textContent = msg.toUpperCase(); el.classList.remove("hidden");
...
function openProductDetail(productId) {
  ...
  if (user && user.preferences) {
    document.querySelectorAll(".size-options span").forEach(s => { s.classList.toggle("active", s.textContent === user.preferences.size); });
  }
...
function handleAuth() {
  ...
  const name = document.getElementById("authName").value.trim(); let usersList = getGlobalUsers();
  ...
  if (isLoginMode) {
    ...
    user = { name, email, password, points: 0, history: [], preferences: { size: "M", color: "#2f4a38" } };
    usersList.push(user); saveGlobalUsers(usersList);
  }
...
function saveActiveSession(updatedUser) {
  user = updatedUser; localStorage.setItem(SESSION_KEY, JSON.stringify(user));
  let usersList = getGlobalUsers(); const index = usersList.findIndex(u => u.email === user.email);
  if (index !== -1) { usersList[index] = user; saveGlobalUsers(usersList); }
}

// Cierre de sesión corregido para heredar el formato "ENTRAR" de tu nueva foto de referencia
function logout() {
  user = null; localStorage.removeItem(SESSION_KEY); document.getElementById("navUserLabel").textContent = "ENTRAR";
  document.getElementById("authView").classList.remove("hidden"); document.getElementById("profileView").classList.add("hidden"); showToast("Terminal desconectada");
}

function savePreferences() {
  if (!user) return;
  user.preferences = { size: document.getElementById("prefSize").value, color: document.getElementById("prefColor").value };
  saveActiveSession(user); showToast("Parámetros actualizados");
}

// Sincronización avanzada de cuenta
function updateAccountCredentials() {
  if (!user) return;
  const newEmail = document.getElementById("updateEmail").value.trim().toLowerCase();
  const newPass = document.getElementById("updatePassword").value.trim(); let usersList = getGlobalUsers();
  ...
  if (newEmail && newEmail !== user.email) {
    ...
    usersList = usersList.filter(u => u.email !== user.email); user.email = newEmail;
  }
  if (newPass) user.password = newPass;
  const idx = usersList.findIndex(u => u.email === user.email); if (idx !== -1) usersList[idx] = user; else usersList.push(user);
  saveGlobalUsers(usersList); localStorage.setItem(SESSION_KEY, JSON.stringify(user));
  document.getElementById("updateEmail").value = ""; document.getElementById("updatePassword").value = ""; renderProfile(); showToast("Credenciales sincronizadas");
}

function renderProfile() {
  if (!user) return;
  document.getElementById("navUserLabel").textContent = user.name.split(" ")[0].toUpperCase();
  document.getElementById("authView").classList.add("hidden"); document.getElementById("profileView").classList.remove("hidden");
  document.getElementById("profileName").textContent = user.name.toUpperCase(); document.getElementById("profileEmail").textContent = user.email;

  if (user.preferences) {
    document.getElementById("prefSize").value = user.preferences.size || "M"; document.getElementById("prefColor").value = user.preferences.color || "#2f4a38";
  }

  const { current, next, progress } = getLevelInfo(user.points);
  document.getElementById("levelBadge").style.background = current.color + "33"; document.getElementById("levelBadge").style.borderColor = current.color;
  ...
  document.getElementById("levelBarFill").style.width = progress + "%"; document.getElementById("levelBarFill").style.background = current.color;
  ...
  if (user.history.length === 0) { historyList.innerHTML = `<li>Sin movimientos todavía.</li>`; } 
  else { user.history.slice().reverse().forEach((h) => { const li = document.createElement("li"); li.innerHTML = `<span>${h.label}</span><span>+${h.points} pts</span>`; historyList.appendChild(li); }); }
}

function subscribe() {
  if (!user) { showToast("Sincroniza terminal primero"); openOverlay("accountOverlay"); return; }
  user.points += 300; user.history.push({ label: "Suscripción Club Oxium", points: 300 });
...
document.addEventListener("DOMContentLoaded", () => {
  renderProducts(); renderCart(); if (user) renderProfile();
  
  // Conteo de stock inteligente y dinámico basado en tu catálogo
